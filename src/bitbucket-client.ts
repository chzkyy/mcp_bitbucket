...
export class BitbucketClient {
  ...
  /**
   * Fetch raw file content from Bitbucket using raw-content-friendly headers.
   * Returns the file text directly, or throws on 404/server errors.
   */
  private async fetchRaw(url: string): Promise<string> {
    const response = await axios.get(`${BitbucketClient.BASE_URL}${url}`, {
      headers: {
        Authorization: this.authHeader,
      ...
      maxContentLength: Infinity,
      maxBodyLength: Infinity,
      // Don't throw on non-2xx so we can return clean error messages
      validateStatus: () => true,
    });

    if (response.status === 404) {
      throw new Error(`File not found (404). Check the path and branch.`);
    }
  ...
  async browseDirectory(
    ...
    try {
      ...
      let url = `/repositories/${ws}/${repoSlug}/src/${branchInfo}`;
  ...
  async getFileContent(
    ...
    try {
      ...
      // Use fetchRaw for reliable raw file content retrieval
      const fileContent = await this.fetchRaw(url);
        ...
        if (matches.length > 0) {
          matchText = matches
            ...
            .map((m: any) => {
              const lines = m.lines || [];
              return lines
                .map((line: any) => `    ${line.prefix || ""}${line.match || ""}${line.supply || ""}`)
                .join("\n");
            })
            .join("\n");
        }
  ...
  async getCommitDiff(workspace: string, repoSlug: string, commitId: string): Promise<string> {
    ...
    try {
      ...
      const diffContent = await this.fetchRaw(url);
